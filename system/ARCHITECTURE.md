...
`state/CURRENT_LEARNING_CONTEXT.md`는 source of truth가 아닌 현재 학습 snapshot이며 Learning Log와 roadmap에서 다시 만들 수 있다. derived state와 source가 다르면 source를 확인하고 충돌을 드러내며, `roadmap/PROGRESS.md`를 자동으로 고치지 않는다.
